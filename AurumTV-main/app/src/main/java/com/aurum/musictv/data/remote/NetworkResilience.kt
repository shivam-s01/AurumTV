...
object NetworkResilience {
     ...
     *
     *  Only checks NET_CAPABILITY_INTERNET (the network claims it has
     *  internet access), NOT NET_CAPABILITY_VALIDATED. VALIDATED requires
     *  Android to have already run its own connectivity probe in the
     *  background, which on TV-box wifi/ethernet is often slow to land or
     *  flaps transiently even while the connection is perfectly usable —
     *  that was causing real playback attempts to be rejected with "You're
     *  offline" while the box was, in fact, online. Dropping VALIDATED
     *  here means a genuinely dead network still fails, just one network
     *  call later (in resolveStreamUrl/retryWithBackoff) instead of here,
